...
fn benchmark_anonymize_email(c: &mut Criterion) {
    ...
    let text = "Contact john.doe@email.com for more information. Also reach out to jane.smith@company.org";
...
fn benchmark_anonymize_multiple_entities(c: &mut Criterion) {
    let anonymizer = Anonymizer::new(vec![EntityType::Email, EntityType::Phone, EntityType::Ssn]).unwrap();
...
fn benchmark_deanonymize(c: &mut Criterion) {
    ...
    c.bench_function("deanonymize", |b| {
        b.iter(|| anonymizer.deanonymize(black_box(&result.anonymized_text), black_box(&result.mapping)))
    });
}

...
describe('Anonymizer', () => {
  ...
  test('deanonymizes correctly', () => {
    ...
    const deanonymized = anonymizer.deanonymize(result.anonymizedText, result.mapping)
  ...
  test('handles duplicate entities', () => {
    ...
    const emailPlaceholders = Object.keys(result.mapping).filter((k) => k.startsWith('EMAIL_'))
...
if (require.main === module) {
  ...
  const tests = [
    () => {
      ...
      if (!result.anonymizedText.includes('EMAIL_')) throw new Error('Email not anonymized')
